...
import { Plus, Search, UserCheck, Clock, TrendingUp, DollarSign, Trash2 } from 'lucide-react';
...
export function StaffManagement() {
  ...
  const loadStaff = useCallback(async () => {
    ...
    try {
      ...
      if (res) {
        const data = (res as { data?: Staff[] })?.data || (Array.isArray(res) ? res : []);
  ...
  const activeStaff = staffMembers.filter((s) => (s.status || 'active') === 'active').length;
  ...
  const totalPayroll = staffMembers.reduce((s, m) => s + parseSalary(m.salary), 0);
                  ...
                  <TableRow key={staff.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium">{staff.username || staff.name}</div>
